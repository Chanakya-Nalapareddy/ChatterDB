...
# -----------------------------
# Conversation state (history-based for follow-ups)
# -----------------------------
@dataclass
class ConversationState:
    """
    Keeps a rolling history so the model can handle follow-up questions.
    We keep it compact to stay inside the context window.
    """
    history: List[Dict[str, Any]] = field(default_factory=list)

    def push_turn(self, question: str, sql: str, result_columns: List[str], row_count: int) -> None:
        self.history.append(
            {
                "question": question,
                "sql": sql,
...
def validate_sql_is_safe(sql: str) -> None:
    ...
    # NEW: prevent fragments (e.g., "d NOT IN (...)")
...
# -----------------------------
# Build LLM messages (full YAML) + conversation history
# -----------------------------
def build_messages(question: str, semantic_yaml_text: str, state: ConversationState) -> List:
    system = (
        ...
        "\n"
        "Conversation rule:\n"
        "- You are given conversation_history (prior questions + SQL). If the new question is a follow-up "
        "(e.g., 'only for...', 'filter to...', 'break it down by...', 'sort by...', 'top N', 'now show...', "
        "'add/remove a condition'), you MUST start from the most recent SQL in conversation_history and modify it.\n"
        "If query is list countries by revenue by countries and genre, then the sql should generate sort countries by revenue and then in turn sort each of these countries by genre\n"
        '''Sales-existence rule (critical):
        - If a question asks for entities that have NEVER sold / have NO sales / have not sold anything,
        you MUST test sales existence using source.InvoiceLine.
        - DO NOT use this incorrect pattern for "never sold":
        LEFT JOIN source.InvoiceLine ... WHERE source.InvoiceLine.InvoiceLineId IS NULL
        because it returns entities that have at least one unsold item even if they have other sold items.
        - Use one of these correct patterns instead:

        Pattern A (preferred): NOT EXISTS
        SELECT ...
        FROM <entity> e
        WHERE NOT EXISTS (
        SELECT 1
        FROM ... JOIN source.InvoiceLine il ...
        WHERE ... = e.<id>
        )

        Pattern B: GROUP BY + HAVING
        SELECT ...
        FROM <entity> e
        LEFT JOIN ... LEFT JOIN source.InvoiceLine il ...
        GROUP BY e.<id>, e.<label>
        HAVING COUNT(il.InvoiceLineId) = 0
        '''
        '''Hierarchical sort rule (critical):
        - If the user asks: "revenue by A and B" AND says:
        "Sort A by total revenue, and within each A sort B by revenue",
        you MUST compute:
        1) metric_by_A_B = SUM(revenue) grouped by (A,B)
        2) total_by_A = SUM(metric_by_A_B) grouped by A
        Then ORDER BY total_by_A DESC, A (tie-break), metric_by_A_B DESC, B (tie-break).

        - Implement this using either:
        (a) CTEs for A_B and A_totals and join them, OR
        (b) a window SUM(metric_by_A_B) OVER (PARTITION BY A) as total_by_A.

        - Do NOT ORDER BY A alphabetically when the user asked to sort A by total revenue.
        '''
    )
    ...
    user_payload = {
        ...
        # NEW: send rolling history for context-window follow-ups
...
# -----------------------------
# Summarize SQL result to natural language
# -----------------------------
def summarize_result_to_nl(question: str, sql: str, df: pd.DataFrame) -> str:
    """
    Generates a natural-language answer grounded in the SQL output.
    Provides only sample rows + computed stats to reduce hallucinations.
    """
    ...
    # small sample for grounding
    sample_rows = df.head(20).to_dict("records")

    # light numeric stats computed locally (safe)
    ...
    system = (
        ...
        "Rules:\n"
        "1) Use ONLY the provided data (row_count, columns, numeric_stats, sample_rows). Do NOT invent values.\n"
        "2) If the result is empty, say no rows matched.\n"
        "3) If the result looks like grouped aggregates (a dimension + numeric metric), mention the top 3 and bottom 3 "
        "based on the main metric if possible.\n"
        "4) Keep it brief (3-6 sentences), with bullet points if it helps.\n"
        "5) If using UNION/UNION ALL, do not place ORDER BY/LIMIT directly before UNION. Wrap each SELECT needing ORDER BY/LIMIT in a subquery or CTE, and apply final ORDER BY only after the UNION.\n"
    )
...
def ask(question: str, semantic_path: Path, duckdb_path: Path, state: ConversationState) -> Dict[str, Any]:
    ...
    # NEW: update rolling history for follow-ups
    state.push_turn(
        question=question,
        sql=obj.sql,
    ...
    # NEW: summarize to natural language
    nl_answer = summarize_result_to_nl(question, obj.sql, df)
...
if __name__ == "__main__":
    ...
    out = ask("Top 10 artists by revenue", SEMANTIC_YAML_PATH, DUCKDB_PATH, state)
    print("SQL:\n", out["sql"])
    print("\nExplanation:\n", out["brief_explanation"])
    print("\nAnswer:\n", out["natural_language_answer"])
